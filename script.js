...
const projects={
 pristine:{title:'Pristine Luxury Camps',hero:'assets/img/pristine.webp',desc:'Experiencia vinculada con hotelería premium y turismo. Desarrollo de comunicación visual, moderación de redes y respuestas para consultas sobre destinos, alojamientos y experiencias.',gallery:['assets/img/pristine_web1.webp','assets/img/pristine_web2.webp','assets/img/pristine_web3.webp'],tags:['Hotelería','Community Management','Moderación','Atención al cliente']},
 ...
 efe:{title:'Agencia EFE',hero:'assets/img/efe.webp',desc:'Producción y adaptación de contenidos para cuentas de distintos rubros dentro de un entorno de agencia. Aplicación de criterios visuales y tono de comunicación según cada marca.',gallery:['assets/img/efe.webp','assets/img/efe2.webp'],tags:['Agencia','Multimarca','Diseño de contenido']},
 luhod:{title:'Luhod LLC',hero:'assets/img/luhod.webp',desc:'Comunicación de servicios empresariales orientados a emprendedores y freelancers. Desarrollo de piezas informativas y contenidos educativos para redes sociales.',gallery:['assets/img/luhod.webp'],tags:['Servicios','Contenido educativo','Redes sociales']}
};
...
document.querySelectorAll('[data-project]').forEach(card=>card.addEventListener('click',()=>{
 ...
 content.innerHTML=`<img class="modal-hero" src="${p.hero}" alt="${p.title}"><div class="modal-body"><h2>${p.title}</h2><p>${p.desc}</p><ul class="tag-list">${p.tags.map(t=>`<li>${t}</li>`).join('')}</ul><div class="modal-gallery">${p.gallery.map((g,i)=>`<img src="${g}" alt="Muestra ${i+1} de ${p.title}" loading="lazy">`).join('')}</div></div>`;
 modal.showModal();
}));
